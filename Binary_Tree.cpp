...
void Binary_Tree<T>::_add(T _data) {
	if (this->_is_empty()) {
		Tree_Node<T>* _data_node = new Tree_Node<T>(_data, NULL, NULL, NULL);
...
void Binary_Tree<T>::_add(Tree_Node<T>* _node, T _data) {
	if (_data < _node->get_data()) {
		if (_node->get_left_node() == NULL) {
			Tree_Node<T>* _data_node = new Tree_Node<T>(_data, NULL, NULL, _node);
...
template<typename T>
template<typename R>
void Binary_Tree<T>::_delete(Tree_Node<T>* _node, R _data) {
	if (_node == NULL) {
		return;
	}
...
*/
template<typename T>
void Binary_Tree<T>::_remove_node(Tree_Node<T>* _node) {
	if (_node->get_left_node() != NULL && _node->get_right_node() != NULL) {
		Tree_Node<T>* _aux = _min_node(_node->get_left_node());
...
*/
template<typename T>
Tree_Node<T>* Binary_Tree<T>::_min_node(Tree_Node<T>* _node) {
	if (_node == NULL) {
		return NULL;
	}
	if (_node->get_right_node() != NULL) {
